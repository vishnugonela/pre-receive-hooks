...
# Author: Scott Ragland <scott.ragland@hpe.com>
# Organization Unit: AI Solutions
# Team: Trellis
#
...
while  read -r old_rev new_rev ref ; do
  ...
  #Ignore the  push if branch/tag got deleted
  ...
  for commit in $(git rev-list "$range" --reverse) ; do
    ...
    if ! echo "$message" | grep -qE "^(ALX|PAD)-[0-9]+"; then
      echo "ERROR"
